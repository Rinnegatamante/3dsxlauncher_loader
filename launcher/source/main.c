...
int main()
{
	...
	// Exit Hooking (Shitty solution, should be made in a better way)
	Handle fileHandle;
	FS_Archive sdmcArchive = (FS_Archive){0x9, (FS_Path){PATH_EMPTY, 1, (u8*)""}};
	FSUSER_OpenArchive( &sdmcArchive);
	Result ret = FSUSER_OpenFileDirectly( &fileHandle, sdmcArchive, fsMakePath(PATH_ASCII, "/pl/state"), FS_OPEN_READ, 0x00000000);
	if (!ret){
		FSFILE_Close(fileHandle);
		FSUSER_DeleteFile(sdmcArchive,fsMakePath(PATH_ASCII, "/pl/state"));
		...
		FSUSER_CloseArchive( &sdmcArchive);
		FSUSER_OpenFileDirectly( &fileHandle, sdmcArchive, fsMakePath(PATH_ASCII, "/pl/state"), FS_OPEN_CREATE | FS_OPEN_WRITE, 0x00000000);
		FSFILE_Close(fileHandle);
		FSUSER_CloseArchive( &sdmcArchive);
	}
	...
	if(sdmcCurrent == 1)
	{
		addDirectoryToMenu(&menu, execPath);
	}
